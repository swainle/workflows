...
test("plans the component test structure before implementing tests", () => {
  ...
  assert.match(component, /每个测试文件使用四级标题并记录测试对象/);
  assert.match(component, /每个测试用例使用五级标题/);
  assert.match(component, /<限界上下文>-<测试层级>-<对象或能力>-<三位序号>/);
  assert.match(component, /`BP`、`Req`、`Design` 引用和必需的 `Given`、`When`、`Then`/);
  ...
  assert.match(testing, /用例描述必须与“用例编号 \+ 简短中文描述”一致/);
  ...
  assert.match(backend, /#### `<领域单元测试文件>`[\s\S]*测试对象：[\s\S]*##### AUTH-DOM-TOKEN-001 <简短中文描述>/);
  assert.match(backend, /BP: `BP-001`[\s\S]*Req: `REQ-001-BR-001`[\s\S]*Given:[\s\S]*When:[\s\S]*Then:/);
  assert.match(backend, /### 接口测试[\s\S]*路径：`test\/integration\/api\/<http、events 或 rpc>\/`[\s\S]*契约：`<OpenAPI、AsyncAPI 或 RPC IDL>`/);
