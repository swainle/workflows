...
test("supports frontend and backend component design modes", () => {
  ...
  assert.match(frontend, /`ux\.md` 内引用页面一律使用 `M-001:P-001`；其他文件引用模块或页面一律使用 `ux:模块:M-001` 或 `ux:模块:M-001:P-001`/);
  assert.match(frontend, /id: M-001:P-002/);
  assert.match(frontend, /ux: ux:模块:M-001:P-002/);
  assert.match(frontend, /> Ref: ux:模块:M-001:P-001/);
  assert.match(frontend, /> Design: ux:表单:FORM-001/);
  assert.match(frontend, /通用可访问性规则只由 `ux:页面规则:GLOBAL` 维护/);
...
test("separates context.md and system.md architecture", () => {
  ...
  assert.match(content, /`system\.md` 容器图 \| `C4Container`；用容器、数据存储与实际通信关系表达/);
});
...
test("lists system.md components by type with development endpoints", () => {
  ...
  assert.match(system, /### web 网页前端/);
  assert.match(system, /### api 后端 API/);
  assert.ok((system.match(/^\| 暴露端口 \| 访问路径 \| 协议 \| 说明 \|$/gm) ?? []).length >= 4);
  assert.match(system, /\| `3000` \| `\/` \| HTTP \| 职责：Web 前端；应用：`apps\/web\/`；设计：`docs\/component\/web\/` \|/);
  assert.match(system, /每个“暴露端口 \+ 访问路径 \+ 协议”组合单独占一行/);
  assert.match(system, /即使端口相同但访问路径不同也必须拆行/);
  assert.match(system, /每行只填写端口、路径、协议和说明，不留空、不合并单元格，四列表格内不使用 `<br>`/);
  ...
  assert.match(system, /\| `3001` \| `\/api\/v1` \| HTTPS\/JSON \|/);
  assert.match(system, /\| `3001` \| `\/api\/v1\/doc` \| HTTPS \|/);
  assert.match(system, /\| `3001` \| `\/api\/v1\/openapi\.json` \| HTTPS \|/);
  assert.match(system, /### worker 异步任务处理[\s\S]*\| `不暴露` \| `不暴露` \| `不暴露` \| 职责：处理异步任务；应用：`apps\/worker\/`；设计：`docs\/component\/worker\/` \|/);
  assert.match(system, /## 容器图\r?\n\r?\n```mermaid\r?\nC4Container/);
  assert.match(system, /System_Boundary\(system, "<系统名称>"\)/);
  assert.match(system, /Container\(proxy, "<代理名称>", "<代理技术>", "统一入口与反向代理"\)/);
  assert.match(system, /Container\(web, "<前端组件>", "<前端技术栈>", "Web 前端"\)/);
  assert.match(system, /Container\(backend, "<后端组件>", "<后端技术栈>", "业务 API 与任务处理"\)/);
  assert.match(system, /ContainerDb\(database, "<数据库名称>", "<数据库技术>", "业务数据"\)/);
  assert.match(system, /ContainerDb\(queue, "<队列名称>", "<队列技术>", "异步任务队列"\)/);
  assert.match(system, /Container\(telemetry, "<遥测平台名称>", "<遥测技术栈>", "可观测性平台"\)/);
  assert.match(system, /Rel\(user, proxy, "访问系统", "HTTPS"\)/);
  assert.match(system, /Rel\(web, backend, "调用 API", "HTTPS\/JSON, Bearer JWT"\)/);
  assert.match(system, /Rel\(backend, database, "读写业务数据", "<数据库访问协议>"\)/);
  assert.match(system, /Rel\(backend, authorization, "授权检查", "<授权协议>"\)/);
  assert.match(system, /用户和外部系统使用 `Person` 或 `System_Ext`/);
  assert.match(system, /每条通信使用实际源、目标之间的 `Rel`/);
  assert.match(system, /C4 图不使用 `Container_Boundary`、`flowchart`、`subgraph`、`direction`/);
  assert.match(system, /### <基础设施名称> <职责类型>/);
  assert.match(system, /\| `<项目确认的暴露端口>` \| `<官方文档确认的访问路径>` \| `<官方文档确认的协议>` \|/);
  ...
  assert.match(system, /每个组件标题下统一使用规定的四列表格/);
