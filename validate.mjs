...
function runSelfTests() {
  ...
  test("require, frontend, backend and docker are optional doc templates", () => {
    ...
    assert.doesNotMatch(frontend, /src\/utils\/load-text\.js|template\.html|style\.css/);
    ...
    assert.match(frontend, /组件私有样式写在 `index\.js` 的模板 `<style>` 中/);
    assert.match(frontend, /Shadow DOM 通过 `var\(--token-name\)` 继承使用/);
    assert.match(frontend, /### 默认演示账户/);
    assert.match(frontend, /`src\/index\.js` 在内存中初始化一个默认演示账户/);
    ...
    assert.match(frontend, /每个 layout、page 和 component 使用独立目录且只包含 `index\.js`/);
    ...
    assert.match(frontend, /组件 CSS 不手写 hex、`rgb\(\)`、`hsl\(\)` 或颜色名/);
    ...
    assert.match(frontend, /### 代码与语法规范/);
    assert.match(frontend, /项目已有的 formatter、linter 和类型检查配置/);
    assert.match(frontend, /JavaScript 只使用浏览器原生 ES6 语法和 ES6 Modules/);
    assert.match(frontend, /禁止 CommonJS、TypeScript、JSX/);
    assert.match(frontend, /HTML 使用小写标签和属性、双引号属性值及合法嵌套/);
    assert.match(frontend, /CSS 使用浏览器支持的标准语法/);
