...
function runSelfTests() {
  ...
  test("require, frontend design, frontend draft, backend and docker are optional doc templates", () => {
    ...
    assert.match(frontendDraft, /draft\/src\/M-001\/P-001\/\*\*/);
    ...
    assert.doesNotMatch(frontend, /文件树校对/);
    ...
    assert.match(frontend, /https:\/\/www\.ruanyifeng\.com\/blog\/2019\/08\/web_components\.html/);
    assert.match(frontend, /<script type="module" src="\.\/src\/index\.js"><\/script>/);
    assert.match(frontend, /原生 Web Components 和 ES6 Modules/);
    assert.match(frontendDraft, /不创建 `component\.json`、`component\.schema\.json`、组件级 `style\.css`、`template\.html`、`load-text\.js`/);
    ...
    assert.match(frontend, /window\.customElements\.define\("m002-p001-appointment-list", AppointmentList\)/);
    assert.match(frontendDraft, /Layout、Page、Component 和共享组件都使用开放 Shadow DOM/);
    assert.match(frontendDraft, /所有颜色和已有 Token 值都通过 `var\(--token-name\)` 使用/);
    assert.match(frontendDraft, /`src\/index\.js` 可在内存中初始化默认演示账户/);
    assert.doesNotMatch(frontend, /export async function loadText\(url\)/);
    assert.match(frontend, /可切换已登录、退出和匿名状态/);
    assert.match(frontend, /不使用 Cookie、`localStorage`/);
    assert.match(frontend, /默认演示账户可完成主要交互/);
    ...
    assert.match(frontend, /\| 页面 \| 路由 \| 布局 \| 目标设备 \| 说明 \|/);
    assert.match(frontend, /\| `M-001:P-001` \| `\/login` \| `ux:M-001:LAYOUT-001` \| `desktop` \| 登录页 \|/);
    assert.match(frontend, /## M-001 身份与认证/);
    assert.match(frontend, /### LAYOUT-001 居中卡片布局/);
    assert.match(frontend, /\| 区域 \| 组件引用 \| 共享内容 \| 显示条件 \|/);
    assert.match(frontend, /导航栏、搜索框、登录\/登出按钮/);
    assert.match(frontend, /`FORM-001`、`DIALOG-001`、`COMP-001`、`S-001` 分别在每个页面内/);
    assert.match(frontend, /跨页引用使用 `ux:M-001:P-001:FORM-001`/);
    assert.match(frontend, /不因展示顺序或条目删除而重排、复用/);
    ...
    assert.match(frontend, /M-001\/P-001\/COMP-001\//);
    assert.doesNotMatch(frontend, /COMP-001-(?:<component>|appointment-list)/);
    assert.match(frontend, /draft-dialog\//);
    assert.match(frontend, /页面尚未生成/);
    assert.match(frontend, /<dialog>`、`showModal\(\)` 和 `close\(\)`/);
    assert.match(frontend, /不把所有状态堆叠在页面内/);
    assert.match(frontendDraft, /`src\/styles\/tokens\.css` 只做机械转换/);
    assert.match(frontend, /只验证组件声明的目标设备/);
    assert.match(frontend, /禁止 `href="#"`/);
    assert.match(frontend, /`aria-invalid` 和 `aria-describedby`/);
    assert.match(frontend, /控制台没有未处理 error/);
    assert.doesNotMatch(frontendDraft, /├─ package\.json|component\.schema\.json 使用|class Store extends|`src\/api\.js` 是/);
    assert.match(frontendDraft, /每个 Layout、Page 和 Component 使用独立目录且只包含 `index\.js`/);
    assert.match(frontendDraft, /不再拆分 App、Registry、Router、Store 或 API 层/);
    assert.match(frontendDraft, /JavaScript 只使用浏览器原生 ES6 语法和 ES6 Modules/);
    assert.match(frontendDraft, /禁止 CommonJS、TypeScript、JSX/);
    assert.match(frontendDraft, /临时数据只放在实际使用它的 Page 或 Component 中/);
    ...
    assert.doesNotMatch(frontend, /:pending|DATA-001|state:M-001:P-001:D-001/);
    ...
    assert.match(frontend, /每个 Draft layout、page 和可复用 component 的 `index\.js` 必须且只能映射一个生产框架组件/);
