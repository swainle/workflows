...
function runSelfTests() {
  ...
  test("require, frontend design, frontend draft, backend and docker are optional doc templates", () => {
    ...
    assert.match(frontendDraft, /Shadow DOM 只用于需要独立复用或强样式隔离/);
    assert.match(frontendDraft, /Light 与 Shadow DOM 样式都通过 `var\(--token-name\)` 使用 Token/);
    ...
    assert.match(frontendDraft, /检查未定义 Token、禁止的原始颜色值、重复 Token/);
    ...
    assert.match(frontendDraft, /`tokens`、`validate:styles`、`dev`、`build` 和 `preview` 脚本/);
    assert.match(frontendDraft, /SPA History fallback/);
    assert.match(frontendDraft, /优先使用 HTML Navigation API/);
    assert.match(frontendDraft, /event\.intercept\(\{ handler \}\)/);
    assert.match(frontendDraft, /统一调用 `navigation\.navigate\(path\)`/);
    assert.match(frontendDraft, /History API fallback/);
    assert.match(frontendDraft, /`getState\(\)`、`setState\(updater\)` 和 `subscribe\(listener, \{ signal \}\)`/);
    assert.match(frontendDraft, /class Store extends EventTarget/);
    assert.match(frontendDraft, /new CustomEvent\("change"/);
    assert.match(frontendDraft, /自己的 `AbortController` 订阅/);
    assert.match(frontendDraft, /不引入第三方状态库或早期 Signals 提案/);
    ...
    assert.match(frontendDraft, /App Shell、Layout 和 Page 默认使用 Light DOM/);
    assert.match(frontendDraft, /`encapsulation` 只能是 `light` 或 `shadow`/);
    assert.match(frontendDraft, /scripts\/build-tokens\.mjs/);
    assert.match(frontendDraft, /scripts\/validate-styles\.mjs/);
    assert.match(frontendDraft, /`design\.tokens\.json` 是全部颜色、字体、间距、尺寸、圆角、阴影和动效的唯一事实源/);
    assert.match(frontendDraft, /任一校验失败时停止/);
