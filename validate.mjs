...
function runSelfTests() {
  ...
  test("require, frontend design, frontend draft, backend and docker are optional doc templates", () => {
    ...
    assert.match(frontendDraft, /不修改 README、`design\.tokens\.json`、`configuration\.md` 或 `testing\.md`/);
    ...
    assert.match(frontend, /`design\.tokens\.json` 是主题唯一事实源/);
    ...
    assert.match(frontend, /禁止手工修改/);
    assert.match(frontend, /目标平台主题由 `<dev 组件>` 直接从 `design\.tokens\.json` 转换/);
    assert.match(frontend, /Frontend 不创建 `ux\.md`、`state\.md` 或 `mapping\.md`/);
    ...
    assert.match(frontendDraft, /CMake 配置与最小增量构建/);
    assert.match(frontendDraft, /WebAssembly 构建并通过本地 HTTP 服务器/);
