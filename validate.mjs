...
test("lists system.md components by type with development endpoints", () => {
  ...
  assert.match(system, /System_Boundary\(system, "系统"\)/);
  assert.match(system, /Container_Boundary\(frontend_layer, "前端组件"\)/);
  assert.match(system, /Container_Boundary\(backend_layer, "后端组件"\)/);
  assert.match(system, /Container_Boundary\(infrastructure_layer, "基础设施组件"\)/);
  assert.match(system, /Container\(worker, "worker", "主要技术栈", "处理异步任务"\)/);
  assert.match(system, /ContainerDb\(redis, "redis", "Redis", "缓存和队列"\)/);
  assert.match(system, /Rel\(web, api, "调用", "HTTPS\/JSON"\)/);
  assert.match(system, /Rel\(api, redis, "读写", "Redis"\)/);
  assert.match(system, /Rel\(api, openfga, "鉴权", "HTTP\/gRPC"\)/);
  assert.match(system, /用户和外部系统使用 `Person` 或 `System_Ext`/);
  assert.match(system, /每条通信使用实际源、目标之间的 `Rel`/);
  assert.match(system, /C4 图不使用 `flowchart`、`subgraph`、`direction`/);
