...
执行提示词必须要求编码 AI 以其中整理的需求信息为准，只读取现有源码，把该平台完成需求所需的源码、测试和文字资源修改生成到 `{{RUN_DIR}}` 中下一个未占用的 `prompt.NN.git.patch`，不得直接应用。复用现有组件与 Tokens；需要改变契约或其他平台时写入 `# 待确认问题`，并标明建议重跑的负责阶段。
