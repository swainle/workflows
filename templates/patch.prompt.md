...
输出一个中间 Git Patch，同时包含全局产物变化和 `{{REQUIREMENT_DIR}}/completion.md`。不得修改业务源码、当前需求的其他文件、其他需求、工作流工具或未在输入上下文中出现的全局文件。即使没有需要同步的全局变化，也必须创建或更新 `completion.md`，不得输出 `no-changes`。
