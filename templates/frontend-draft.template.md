...
- 只创建或修改该页面的 `draft/src/modules/M-001/P-001/**` 及其引用的 `draft/src/modules/M-001/LAYOUT-*/**`，以及让该页面可预览所必需的
  `draft/package.json`、`draft/component.schema.json`、`draft/index.html`、`draft/src/index.js`、`draft/src/api.js`、`draft/src/data/**`、`draft/src/app/**`、
...
- Draft 是可独立安装、启动、构建和预览的完整前端项目。`package.json` 至少提供 `dev`、`build` 和 `preview` 脚本；
...
- `src/styles/tokens.css` 由 `design.tokens.json` 机械转换为 CSS Custom Properties；Shadow DOM 通过 `var(--token-name)` 继承使用。
...
- `tagName`、`entry`、`targetDevice`、输入、输出、Slot、状态引用和依赖只在 `component.json` 登记；代码和 `mapping.md` 不维护第二份契约。
...
- 使用 History API；应用内导航统一调用 `router.navigate(path)`，由 Router 执行 `history.pushState()` 并渲染页面。
- 监听 `popstate`，保证浏览器后退和前进可以恢复页面；直接刷新任一已登记路由时，开发服务器通过 SPA fallback 返回 `index.html`。
- Router 负责路径标准化、精确匹配、当前 Route 写入 Store、Layout 与 Page 装配以及权限守卫，不负责读取页面数据。
- 页面链接使用真实 `href`；同源且命中 Route Table 的导航由 Router 接管，修饰键、新窗口、下载和外部链接保留浏览器默认行为。
...
- `src/app/store.js` 提供单一共享 Store，至少包含 `getState()`、`setState(updater)` 和 `subscribe(listener)`；每次更新产生新快照，禁止组件直接修改内部对象。
...
- Store 更新不可变且订阅可释放；组件断开或请求被替换时取消未完成的数据请求，不产生过期渲染。
- 未生成页面和适用运行状态能通过弹窗显示；共享文件未破坏已有页面。
- 所有颜色来自 Token，目标设备表现正确，键盘、焦点、语义、表单错误关联和动效降级已检查。
