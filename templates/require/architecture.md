...
| 文件 | 唯一维护内容 |
|---|---|
| `context.md` | 系统边界、用户、使用环境、目标终端、语言、外部系统和主要关系 |
...
只回答系统服务谁、解决什么问题、用户在什么环境使用、依赖哪些外部系统。固定结构：
...
| 用户 | 目标 | 使用环境 | 目标终端与输入方式 |
|---|---|---|---|
| `<角色>` | `<业务目标>` | `<地点或业务场景>` | `<终端类别、方向及触摸/键盘等输入方式>` |

## 语言与内容环境

- 默认语言：<已确认语言>
- 日期与时间习惯：<已确认格式>
- 影响界面设计的内容范围：<已确认的典型或最大长度、数据量；不适用时省略>

## 全局体验基线

- <跨模块共享且已确认的可访问性、隐私或状态表达要求；具体可测阈值由相关 NFR 定义>
...
不出现内部组件、端口、数据库、框架、部署拓扑、颜色、字体、间距、Layout 或 Component。未知的终端、语言、内容规模或体验约束会影响设计结果时，通过对话一次确认一个问题，不猜测；没有适用内容时省略对应条目。
