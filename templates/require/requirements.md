...
模块在当前组件内使用连续且唯一的 `M-001`、`M-002`。每个模块内的 FR、BR、FLOW、NFR、PERM 和 AC 分别从 `001` 独立递增分配，
新增项使用同类历史最大编号加一；例如 `M-001/FR-001` 与 `M-002/FR-001` 可以同时存在。同模块内可以使用 `FR-001` 等短标识；
跨模块引用必须使用 `M-001/FR-001` 等限定标识。已有编号不因 Issue 或条目的增删而重排或复用，删除后允许保留编号空缺。
...
- 每个模块使用 `### M-<三位编号> <模块名称>` 三级标题和一张独立 FR 表；模块和表内 FR 按编号升序，首个 FR 使用 `001`，新增 FR 使用历史最大编号加一，不填补空缺。
