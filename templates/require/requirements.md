...
- 产品与领域专家：确认目标、范围、角色、使用环境、模块、页面、FR 和业务语言。
- 权限与风险专家：确认 PERM、NFR、安全、隐私、数据可见范围、边界和例外。
- 验收专家：确认 FLOW、AC、TC、页面可观察状态、Mermaid 追溯图和可执行场景。
...
4. 对面向界面的需求建立设计输入视图：README 提供页面关系，FR 提供输入与结果，FLOW 提供操作顺序，AC 提供实际存在的 loading、empty、error、disabled、success、unauthorized 等可观察状态，NFR 提供可测终端、响应式、可访问性和性能约束，PERM 提供数据可见范围，`context.md` 提供用户、终端、语言和业务环境。不得机械补齐不存在的状态或猜测阈值。
5. 再读取受当前需求输入影响 FR 的 Mermaid 直接关联 BR、FLOW、NFR、PERM、AC 和 TC；不无差别加载全部关联文件。
6. 专家团分别对比当前需求输入、全量 CRUD 视图与已有事实；主 Agent 合并结论、去重并解决专家间的分歧。
7. 每项能力在写入前必须得到唯一分类：
...
8. `CONFLICT` 或专家分歧会改变行为、契约、安全、数据、终端范围、语言、可访问性或兼容性时，在写入前一次询问一个关键问题；其他情况由主 Agent 选择最小一致方案。
9. 按分类增量更新事实源，最后同步 `README.md` 索引和 FR Mermaid 投影。
...
- README 只维护页面、角色、入口、直接导航和 FR 映射；页面控件、视觉值、Layout 和 Component 不属于需求索引。
...
每个 FR 固定保留主体、前置条件、输入、成功结果和失败结果五个字段。成功和失败结果必须是角色可观察的业务结果，可作为页面内容与状态输入，但不规定控件、文案排版或视觉实现。追溯图只放实际相关节点。
...
涉及页面时，FLOW 只说明角色如何开始、经过哪些业务操作及如何结束，不指定导航组件或页面布局。
...
- 目标终端、最小视口、键盘操作、焦点、对比度、字号缩放、响应式结果和性能只有在产品需要且可测量时才写入 NFR。
- 全局用户环境、终端类别和语言属于 `context.md`；模块 NFR 只保存本模块的指标、阈值和验证条件，不重复全局事实。
- 无法从需求输入确定阈值时通过对话确认，不使用行业惯例猜值。
...
页面设计只消费 PERM 中的角色、资源、动作、范围和允许条件，用于决定数据是否可见、可操作或需要隐藏；PERM 不指定控件状态或视觉样式。
...
- loading、empty、error、disabled、success、unauthorized 等状态只在业务或外部依赖确实可能产生且用户需要观察或处理时写成 Scenario；不得为凑齐状态而创建虚假 TC。
...
- 面向界面的需求可以从 README、FR、FLOW、AC、NFR、PERM 和 `context.md` 还原页面关系、业务内容、操作顺序、可观察状态、可测体验约束、数据可见范围和使用环境。
