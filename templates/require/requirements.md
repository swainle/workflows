...
模块在当前组件内使用连续且唯一的 `M-001`、`M-002`。每个模块内的 FR、BR、FLOW、NFR、PERM 和 AC 分别从 `001` 独立连续编号，
例如 `M-001/FR-001` 与 `M-002/FR-001` 可以同时存在。TC 使用 `AC-001-TC-001`，在所属 AC 内从 `001` 独立编号。
同模块内可以使用 `FR-001` 等短标识；跨模块引用必须使用 `M-001/FR-001` 等限定标识。已有编号不因 Issue 或其他条目的增删而重排。
...
- 每个模块使用 `### M-<三位编号> <模块名称>` 三级标题和一张独立 FR 表；模块按编号升序，每张表内的 FR 从 `001` 开始独立连续编号。
...
- TC 按 `AC-<AC>-TC-<TC>` 编号，在每个 AC 内从 `001` 独立连续；跨模块使用带 `M-<模块>` 前缀的 Gherkin 标签，已有 TC 不重排。
- `Examples` 行只是同一 TC 的数据变体，不创建新 TC 编号。
- Given/When/Then 使用业务语言，不写数据库、框架或内部实现断言。

## 完成检查

- `README.md` 包含组件职责、角色索引和按模块分组的 FR 索引，每个 FR 编号都对应所属模块内唯一的 `M-*/FR-*.md`，所有 Issue 来源链接真实、去重且完整。
