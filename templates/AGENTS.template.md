...
- `req <需求组件>`：读取该组件根目录的全部文件，但不自动读取 `REQ-*` 子目录。
- `req <需求组件> issue <编号>`：在上述文件之外读取唯一匹配的
  `REQ-<至少三位编号>-*` 目录。编号不足三位左补零，三位以上不截断。
- `<doc 组件> issue <编号>`：加载 Issue 需求模板，在当前文档组件中创建或更新匹配的
  `REQ-*` 目录；这里的组件名没有保留值，例如 `require` 只是普通名称。
...
- `issue` 紧跟 `req <组件>` 时限定被引用组件；没有 `req` 时作用于当前组件并加载 Issue 模板。
  它可以与 `tmp arch` 同时使用，例如 `<doc require> tmp arch issue 1` 同时维护全局设计和该 Issue。
  一条指令只允许出现一个 `issue`。

组件名只能包含字母、数字、点、下划线和连字符。保留全局指令优先于组件名；格式错误、
未知参数、引用组件不存在、Issue 未唯一匹配或应用目录缺失时停止并说明原因。
