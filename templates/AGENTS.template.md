...
2. 提示词读取不属于实质操作；完成读取后，在读取或修改项目业务文件、运行项目命令、调用外部系统、
   提出澄清问题或执行其他任务动作前，先回显本次命中的规则编号。
3. 第一条用户可见回复必须以单独一行输出：

```text
命中规则：AI-001、AI-005、AI-006、AI-011、AI-<STAGE>-001
```

4. 只列出条件与当前指令实际匹配的规则，不列出未命中规则，不虚构编号。
5. 编号按根提示词、阶段提示词、模式提示词的顺序排列；同一编号只列一次。
6. 阶段提示词规定全部功能都必须执行时，列出该阶段全部适用编号；条件性功能只在条件成立时列出。
7. 同一条用户指令只需在首次回复回显一次；用户提出新的受管理指令时重新计算并回显。
8. 未命中本工作流的普通自然语言任务不强制回显规则编号。
`````
