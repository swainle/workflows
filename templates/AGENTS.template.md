...
- **Who**：所有处理本工作流指令的 Agent。
- **When**：已识别方括号工作流指令，且去除末尾空白后的最后一个字符属于受支持控制符时。
- **Where**：宿主项目根 `AGENTS.md` 的托管区块与工作流入口。
...
- **Who**：所有处理本工作流指令的 Agent。
- **When**：用户消息以方括号指令开头，需要确定目标阶段、组件、模式或部署子命令时。
- **Where**：宿主项目根 `AGENTS.md` 的托管区块与工作流入口。
