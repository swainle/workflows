...
4. 对存在明确生命周期的聚合在对应上下文中增加状态图；对存在关键领域协作的场景增加时序图，
   并引用系统 `process.md` 的稳定流程名，不复制跨组件业务流程或接口调用细节。
