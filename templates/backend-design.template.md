...
##### AUTH-DOM-TOKEN-001 <简短中文描述>

BP: `BP-001`

Req: `REQ-001-BR-001`

Given: <执行前已经成立的状态、输入和依赖条件>

When: <执行一个公开领域行为>

Then: <可观察的返回值、状态、领域错误或领域事件>
...
##### AUTH-INT-SESSION-001 <简短中文描述>

Req: `REQ-001-BR-002`

Given: <真实数据库状态和迁移前置条件>

When: <执行 Repository 操作>
...
##### AUTH-INT-ADAPTER-001 <简短中文描述>

Design: `interface.md#<章节>`

Given: <真实或受控外部边界状态>

When: <执行 Adapter 公开操作>
...
- 开头只用一句话说明当前语言、运行时、构建或包管理工具和测试框架，不创建语言映射表或
  ...
  使用 `pnpm` 和 Vitest。
- 每个测试文件使用四级标题并记录测试对象；每个测试用例使用五级标题，格式为
  “`<用例编号> <简短中文描述>`”。编号格式为
  “`<限界上下文>-<测试层级>-<对象或能力>-<三位序号>`”，在组件内唯一且稳定，删除后不复用。
- 每个用例使用可选的 `BP`、`Req`、`Design` 和必需的 `Given`、`When`、`Then`；三类来源
  至少存在一项。`BP` 只引用系统 `process.md` 中实际存在的跨组件业务流程，`Req` 引用实际
  需求项，`Design` 引用实际设计文件及章节；存在多个引用时使用逗号分隔，不适用时省略该行。
