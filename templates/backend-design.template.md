...
- 状态图和时序图放在对应上下文，不创建独立 `state.md`、`sequence.md` 或组件级 `process.md`；每个时序图先使用 `- **<流程>**` 简短说明引用对象，再使用 `> Ref: process:业务流程:<BP 编号>` 引用实际系统流程，最后绘制 Mermaid，不复制跨组件调用顺序。存在多个流程时按相同结构依次记录。
