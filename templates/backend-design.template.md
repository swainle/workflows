...
- 每个具体范例都必须就近包含固定提示 `> - 根据实际情况修改`，并可继续说明具体调整范围；执行时保留明确标记为固定的标题与顺序，
...
3. 根据 AI-BACKEND-001 的客观条件选择设计强度，并在 `component.md` 概述记录结论与触发事实。
4. 完整 DDD 模式在 `ddd.md` 中按限界上下文分章，用领域结构图展示应用服务、领域服务、聚合根、
