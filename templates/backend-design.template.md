...
3. 用 `ddd.md` 先确定上下文边界、业务能力、统一语言、聚合根、不变量、事务边界和必要领域事件；
   只有理解业务规则确实需要时才列出核心实体和值对象。
...
| 聚合 | 聚合根 | 核心成员 | 业务不变量 | 事务边界 |
|---|---|---|---|---|
| <聚合> | <聚合根> | <必要实体和值对象> | <始终成立的业务规则> | <一次事务允许修改的范围> |

## 核心模型

| 名称 | 类型 | 身份或等价规则 | 所属聚合 | 业务职责 |
|---|---|---|---|---|
| <名称> | 实体或值对象 | <身份或等价判断> | <聚合> | <业务职责> |
...
- 一个组件默认对应一个上下文边界；只有确实存在不同统一语言和模型边界时才增加上下文行。
- 每个实际聚合必须列出聚合根、不变量和事务边界；“核心成员”只列理解聚合所需的实体和值对象。
- “核心模型”只列具有业务身份、生命周期、重要等价规则或领域约束的实体和值对象；
  没有此类模型时删除该章节。
