...
| 权限 | 主体或角色 | 资源 | 条件 | 数据范围 | 执行点 |
|---|---|---|---|---|---|
| `<PERM编号>` | <主体> | <资源> | <条件> | <范围> | <Middleware、Policy 或 Handler> |
...
- “权限控制”不重复 `interface.md` 中操作到权限的映射；权限编号必须存在于需求，关系授权模型由 `authorization.fga` 维护，本文件只定义主体、资源、条件、数据范围和执行点。
