...
生成任何阶段产物前，先检查对需求目标、范围、约束、验收条件和本阶段关键决策的把握。任一项置信度低于 95% 时，必须暂停生成文件并与使用者对话确认：

- 每次只提出一个当前影响最大的问题，等待使用者回答后再继续；不得一次罗列多个问题。
- 根据回答重新评估，仍有低于 95% 的事项时继续逐项提问。
- 不得用自行假设、默认选项或仅标记“待确认”代替对话。
- 全部关键事项达到 95% 后，才可以生成阶段产物和 AI 结果文件。
...
- 以需求、现有产物和可验证事实解决分歧；无法确认时遵守“逐项确认”，不得编造。
- 最终只提交经过逐项确认的统一方案，不输出内部讨论过程。
