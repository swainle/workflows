...
生成阶段产物前，检查对需求目标、范围、约束、验收条件和本阶段关键决策的把握。任一项置信度低于 95% 时，不在对话中暂停或提问，继续生成已确认的内容，并把所有不确定项写入本阶段一个允许修改的 Markdown 产物：

```md
# 待确认问题

- [ ] 问题：<只问一个明确事项>
  - 原因：影响 <阶段> <内容>；<为什么需要确认，必要时说明建议重跑的阶段>
  - 参考：<推荐答案> / <备选答案> / <备选答案>
  - 回答：
```

- “参考”提供一至三个有依据的简短答案，推荐项放在最前；它只帮助理解，不代表使用者已经确认。
- 不得自行填写“回答”，不得用参考答案、假设或默认选项隐藏问题。
- 信息应由其他阶段产出或修改时，在“原因”中明确负责阶段和重跑建议。
- 再次执行本阶段时，读取使用者手动填写的回答，将确认结论整合到产物正文，移除已解决的问题，再重新检查是否仍有不确定项。
- 每个阶段 Markdown 产物都保留 `# 待确认问题`；没有未确认问题时写“无”。
- 存在待确认问题时只生成供合并和填写的阶段产物，不得把未确认内容写成确定结论。
...
- 以需求、现有产物和可验证事实解决分歧；无法确认时遵守“文件式确认”，不得编造。
...
存在待确认问题时必须生成阶段 Git Patch，并将分析文件的 `result` 写为 `needs-confirmation`。问题为“无”且有修改时写为 `proposed`。
...
result: <proposed 或 needs-confirmation>
---
