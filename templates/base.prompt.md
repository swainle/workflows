...
- 缺失信息可以由使用者直接确认时，继续逐项提问；若该信息应由其他阶段产出或修改，停止当前阶段，明确说明负责阶段、缺失内容和重跑原因，并要求使用者直接运行 `pnpm -s work:<阶段> --require "<需要补充的信息>"`。
- 要求重跑其他阶段时，不得生成任何结果文件，也不得继续使用当前 Prompt；上游阶段完成后必须重新生成本阶段 Prompt，以读取更新后的内嵌上下文。
