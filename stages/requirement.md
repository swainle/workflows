...
- **Who**：处理 `[<编号>]` 指令的需求分析 Agent。
- **When**：成功识别 `[<Issue编号>]` 并需要确定本阶段目标与不包含范围时。
- **Where**：GitHub Issue 与当前需求目录。
...
| `docs/requirements/REQ-<至少三位Issue编号>-*/**` | 允许 | 允许 | 允许 | 允许 |
...
- **Who**：处理 `[<编号>]` 指令的需求分析 Agent。
- **When**：任务所需文件、事实、决策或操作超出需求阶段职责或权限时。
- **Where**：GitHub Issue 与当前需求目录。
...
- 产品价值：目标用户、业务目标、成功信号和不包含范围是否明确。
- 业务一致性：术语、规则、流程、异常分支和数据口径是否互相一致。
- 系统边界：外部参与者、系统责任、跨组件影响和约束是否明确且未提前设计实现。
- 可验收性：每项功能和规则是否存在可观察、可判定且可追溯的验收条件与测试场景。

不适用的视角可跳过；发现必须写入对应需求条目或在最终回复中列为阻塞，不生成独立评审报告。

## AI-REQUIREMENT-005

- **Who**：处理 `[<编号>]` 指令的需求分析 Agent。
- **When**：创建、引用、排序或增量维护任何需求条目编号时。
- **Where**：GitHub Issue 与当前需求目录。
- **What**：提供“编号”功能；具体规则、格式和约束如下。
- **Why**：确保需求事实可追溯、格式稳定且不会越过需求阶段边界。
...
- **Who**：处理 `[<编号>]` 指令的需求分析 Agent。
- **When**：已确认需求并准备创建或更新正式需求产物时。
- **Where**：GitHub Issue 与当前需求目录。
- **What**：提供“固定格式”功能；具体规则、格式和约束如下。
- **Why**：确保需求事实可追溯、格式稳定且不会越过需求阶段边界。
...
- **Who**：处理 `[<编号>]` 指令的需求分析 Agent。
- **When**：前置条件满足并准备执行需求阶段任务时。
- **Where**：GitHub Issue 与当前需求目录。
- **What**：提供“执行步骤”功能；具体规则、格式和约束如下。
- **Why**：确保需求事实可追溯、格式稳定且不会越过需求阶段边界。

1. Issue 编号不足三位时左侧补零，达到或超过三位时保留完整编号，不截断、不取模。
