...
- 每个组件只占一行且名称不得重复；同一组件存在多个端口、路径或协议时，在对应单元格中使用 `<br>` 按相同顺序逐项对齐。
- 自研组件名称对应 `[<组件>]` 指令；“说明”固定使用“职责：<职责>；应用：`apps/<路径>/`；设计：`docs/component/<路径>/`”，
  应用和设计路径不包含 `..` 且不得重复。基础设施的“说明”固定包含用途、`technology.md` 中的具体版本和对应版本官方文档链接，不登记应用或设计目录。
...
- Swagger UI、OpenAPI、AsyncAPI 和其他对外接口文档按实际情况作为同一组件的附加端口、路径和协议逐项对齐登记。
...
- 官方文档声明的默认或监听端口不等于项目实际暴露端口；表格只填写用户或项目系统事实已确认的开发环境映射，并在“说明”中链接官方依据。
- 已确认启用的管理界面作为同一基础设施的附加端口、路径和协议登记；认证状态、凭据来源和本地初始化方式简写在“说明”中。
  未启用或官方文档未提供管理界面时不虚构对应端点。
- 管理界面未启用认证时在“说明”中明确写“开发环境无认证”，不虚构账号、密码或初始化步骤；需要认证时，
  凭据来源只记录环境变量名、密钥引用、初始化脚本或 Runbook 位置。
...
| 组件名称 | 暴露端口 | 访问路径 | 协议 | 说明 |
|---|---|---|---|---|
| `api` | `3001`<br>`3001`<br>`3001` | `/api/v1`<br>`/api/v1/doc`<br>`/api/v1/openapi.json` | HTTPS/JSON<br>HTTPS<br>HTTPS | 职责：业务 API、Swagger UI 和 OpenAPI 契约；应用：`apps/api/`；设计：`docs/component/api/` |
...
| 组件名称 | 暴露端口 | 访问路径 | 协议 | 说明 |
|---|---|---|---|---|
| `<基础设施名称>` | `<项目确认的暴露端口>` | `<官方文档确认的访问路径>` | `<官方文档确认的协议>` | 用途：<实际用途>；版本：`<technology.md 中的具体版本>`；官方文档：[对应版本官方文档](<URL>) |
````

- “组件清单”始终存在；所有组件分类统一使用规定的五列表格，组件目录、职责、基础设施版本和官方依据按固定格式写入“说明”。
...
- 组件清单按类型使用三级章节；所有分类只有“组件名称”“暴露端口”“访问路径”“协议”“说明”五列表格，组件名称和路径合法且不重复。
- 自研组件的“说明”包含职责、应用目录和设计目录；基础设施的“说明”包含用途、具体版本和对应版本官方文档链接。
- 只登记已确认的开发环境实际暴露端口、路径和协议；确认不暴露时三个字段一致，未知信息没有使用猜测值，测试和生产地址不写入 C2。
- 对外文档只包含实际暴露的 Swagger UI、OpenAPI、AsyncAPI 或其他文档，端口、路径和协议按相同顺序对齐。
