...
`openapi.json` 只维护跨组件同步 HTTP 调用：每个 `operationId` 必须唯一、稳定且归属一个提供方；
...
- `docs/system/openapi.json` 是跨组件同步 HTTP 契约唯一源文件；Backend 实现它，Frontend 通过稳定 `operationId` 消费它并用其 Schema 或示例驱动 Mock。`asyncapi.json` 仍由提供方组件维护。
...
- **What**：定义 C1 系统上下文、C2 容器关系和运行组件清单格式。
...
- `openapi.json` 是全部跨组件同步 HTTP 调用的唯一契约源；每个 `operationId` 唯一且稳定，包含请求、响应、错误、安全要求和适用示例。Backend 实现它，Frontend 消费它并以其驱动 Mock；不创建第二份 HTTP 契约。
