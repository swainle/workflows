...
确认并维护系统架构、组件清单、技术栈、跨组件业务流程、同步 HTTP 契约、安全、可观测性和 Git 工作流。

不负责具体需求、组件内部设计、源码、测试实现或部署；`openapi.json` 是跨组件同步 HTTP 契约的例外。
...
组件界面、Token 和内部规范切换 `<组件>`；同步 HTTP 契约切换 `<system>`；实现切换 `<组件 dev>`；部署切换 `<deploy>`。
...
| 文件 | 作用 | 创建条件 | 可修改内容 |
|---|---|---|---|
| `context.md` | 系统上下文 | 始终 | 用户、外部系统、系统职责和外部关系 |
| `system.md` | 组件清单和容器图 | 始终 | 组件目录、对外文档、主要技术栈和容器通信 |
| `technology.md` | 语言、框架、数据库和版本策略 | 始终 | 全局技术选型 |
| `openapi.json` | 跨组件同步 HTTP 契约 | 存在同步 HTTP 调用 | 路径、稳定 operationId、Schema、错误、安全要求和示例 |
| `process.md` | 分类维护跨组件业务流程 | 存在跨组件业务流程 | 业务时序图 |
...
`openapi.json` 只维护跨组件同步 HTTP 调用：每个 `operationId` 必须唯一、稳定，并通过
`x-provider: <组件名称>` 指定一个已登记的提供方；
路径、请求、响应、错误、安全要求和适用示例是唯一事实。示例必须符合对应 Schema，并覆盖适用的成功、
空数据和失败响应；Frontend Mock 只能按同一 `operationId` 使用这些 Schema 或示例，不得新增契约字段、
状态码或错误码。破坏性变更必须先更新该文件，再更新提供方实现、消费方设计和测试。

## AI-SYSTEM-006

- **Who**：处理 `<system>` 指令的系统设计 Agent。
- **When**：新增、删除、拆分、合并或修改系统组件及其目录、地址或连接信息时。
- **Where**：`docs/system/system.md`。
...
- Swagger UI、OpenAPI、AsyncAPI 和其他对外接口文档按实际情况作为同一组件的附加端口、路径和协议组合逐行登记；这些行只表示运行时暴露地址。
- `docs/system/openapi.json` 是版本库中跨组件同步 HTTP 契约唯一源文件，不等同于 `system.md` 中登记的运行时 URL。每个操作以 `x-provider` 对应一个组件清单中的 Backend；Backend 实现它，Frontend 通过稳定 `operationId` 消费它并用其 Schema 或示例驱动 Mock。`asyncapi.json` 仍由提供方组件维护。
...
- 管理界面未启用认证时不虚构账号、密码或初始化步骤；需要认证时，`system.md` 组件清单的“账密”只引用
  `security.md`“凭证”表中已登记的变量名。
- `system.md` 组件清单和除 `security.md`“凭证”表之外的版本控制文件不得记录凭证值；任何文件都不得记录测试、生产凭证、
  官方默认凭据、令牌、证书或私钥，也不得用看似真实的占位值代替。
- 测试和生产凭据必须由 `<deploy>` 通过独立环境配置或密钥系统注入，不得复用开发凭据来源。
- `system.md` 组件清单只记录开发环境端口、路径和协议；测试、生产及其他环境由 `<deploy>` 维护。
...
- **Where**：`docs/system/context.md` 与 `docs/system/system.md`。
- **What**：定义 `context.md` 系统上下文、`system.md` 容器关系和运行组件清单格式。
- **Why**：避免系统边界、容器职责和组件登记相互矛盾或重复表达。

`context.md` 只保存长期有效的系统上下文：
...
`system.md` 先保存容器图，再保存组件清单和开发接口信息：
...
- 路由、中间件、源码目录、数据表、组件私有契约和同步 HTTP 契约的实现映射放入对应组件设计目录；跨组件同步 HTTP 契约本体只维护在 `openapi.json`。
- `system.md` 组件清单只记录实际暴露的开发环境端口、路径和协议；测试、生产等环境以及构建、发布和部署方式切换 `<deploy>`，写入项目根目录的 `deploy/`。

## AI-SYSTEM-008

- **Who**：处理 `<system>` 指令的系统设计 Agent。
- **When**：存在需要长期维护的跨组件业务流程，因而创建或更新流程设计时。
- **Where**：`docs/system/process.md`。
- **What**：定义按角色分组的 BP 编号、描述、需求引用和跨组件时序图格式。
- **Why**：避免同一业务流程被多个组件重复描述，并保持 BP 与需求引用稳定。

`process.md` 只维护跨组件业务流程，按实际用户角色分组并使用 `sequenceDiagram`：
...
- 二级标题按相关需求和系统上下文中的实际角色创建，不预设固定角色；多个角色都能独立发起且行为一致的流程归入“通用”，没有用户发起的自动流程归入“系统”。
- 跨组件业务流程使用只含 `BP-<三位序号>` 的三级标题；BP 编号在整个 `process.md`
  中唯一且保持稳定，新流程按最大编号递增，删除后不复用，重命名或调整流程时不改变编号。
- 每个 BP 标题后使用连续引用行 `> Desc: <流程简述>` 和 `> Ref: <需求编号>`；`Desc` 简短表达业务目标，`Ref` 引用实际存在的完整 FR、BR、AC 或 PERM 编号，多个编号使用顿号分隔。
- 每个流程正文只保留 `Desc`、`Ref` 和时序图；参与者、参与组件、调用顺序和可观察结果直接由图表达，
  不再重复填写目标、触发条件、参与者、参与组件、成功结果或失败结果。
- 时序图只展示用户、外部系统和 `system.md` 组件清单中真实存在的组件，每条消息只表达一次跨组件业务交接；
  同一组件内部的连续步骤合并，不展示方法、类、内部模块、协议细节或普通技术异常。
- 默认只画主成功路径。只有失败会改变跨组件协作、触发补偿或产生独立业务结果时才使用 `alt`；
  普通校验失败、接口错误和组件内部异常交给需求、接口或组件设计。
- BP 通过 `Ref` 引用实际存在的 FR、BR、AC 或 PERM；阈值、计算、领域不变量和判断条件
  仍以需求项为准，不在流程中重新定义。图过长时按独立业务目标拆分为多个 BP。
- 多角色业务流程只归入主要发起角色，其他角色直接作为时序图中的 `actor`，不重复流程。
- 没有“通用”或“系统”流程时不创建对应二级标题。
- 构建、CI/CD、发布、部署和回滚流程不写入 `process.md`，统一由 `<deploy>` 阶段设计和维护。
...
- **Where**：`docs/system/gitflow.md`。
- **What**：定义分支类型、提交、合并、发布、Hotfix 和版本流转规则。
- **Why**：避免开发、发布和紧急修复使用互相冲突的分支与版本路径。
...
- **Where**：`docs/system/technology.md`。
- **What**：定义开发技术、运行组件、容器镜像、版本约束和官方文档索引。
- **Why**：避免组件自行选择不兼容技术或依赖无法追溯的版本与资料。
...
- 运行组件名称必须存在于 `system.md` 的组件清单，每个运行组件一行。
...
- “组件文档”只包含“开发技术文档”和“组件技术文档”两个四级章节，不增加混合清单。
- 开发技术文档只使用“技术名称”“文档”“说明”三列；技术名称同时写明具体版本或 `<主版本>.x`，
  覆盖语言、框架、包管理器、构建和测试工具等实际开发依赖。
- 组件技术文档只使用“组件名称”“文档”“说明”三列；组件名称必须存在于“运行组件”表，
  只登记具有独立技术文档的实际组件，不为自研组件虚构官方文档。
- 两类“文档”都必须链接对应版本的官方文档，不使用博客、搜索结果或非官方教程；版本不使用
  `latest`、`*` 或完全不限定范围。
...
- **Where**：`docs/system/security.md`。
- **What**：定义系统安全目标、信任边界、全局控制、凭证变量和风险例外。
- **Why**：避免组件重复定义安全原则，并防止凭证值和系统级控制散落到下游文件。
...
- “凭证”集中登记项目已确认的开发环境变量、值和说明；`system.md` 组件清单只引用变量名。不得猜测值，不登记测试、生产凭证，
  不登记令牌、证书、私钥或其他不应进入版本库的秘密；这些值由 `<deploy>` 通过环境配置或密钥系统注入。
- 信任边界图的节点使用 `context.md` 和 `system.md` 中的实际用户、外部系统及组件名称，主关系从左到右，
...
- **Where**：`docs/system/observability.md`。
- **What**：定义系统级日志、指标、追踪、告警、数据治理和例外要求。
- **Why**：避免各组件产生无法关联的遥测数据，并保持系统运行目标可验证。
...
- 遥测链路图使用 `system.md` 中的实际组件和 `technology.md` 中已确认的共用平台，主链路从左到右，
...
2. 更新 `system.md` 组件清单时，逐个检查 `technology.md` 中的基础设施，并查阅对应版本官方文档确认服务端口、路径、协议、管理界面和启用条件；
...
- `openapi.json` 是全部跨组件同步 HTTP 调用的唯一契约源；每个 `operationId` 唯一、稳定且以 `x-provider` 指定已登记的 Backend，包含请求、响应、错误、安全要求和适用示例。Backend 实现它，Frontend 消费它并以其驱动 Mock；不创建第二份 HTTP 契约。
...
- `technology.md` 的运行组件只包含组件名称、具体版本容器镜像 Tag 和说明，组件名称与 `system.md` 一致；
  组件文档分为开发技术文档和组件技术文档，两类均使用三列表格并链接对应版本官方文档；非开发环境全部容器化。
