...
- 测试目录按实际需要使用 `fixtures/`、`support/`、`unit/`、`integration/` 和 `concurrency/`；
  不存在对应测试层级时不创建空目录。
...
- `testing.md` 必须包含“测试命令”章节，按适用测试层级记录命令、前置条件和执行范围，
  并包含单个文件、单个用例等局部执行方式。
- `testing.md` 的每个测试分类先写目录路径，再用表格逐行记录测试文件、测试对象、
  函数或操作、单个测试用例和测试事项；文件名相对于该分类路径。
- 一行只描述一个稳定测试用例；“测试用例”写行为场景，“测试事项”写规则、边界、
  错误和副作用，不使用通配函数名或把多个场景合并为一行。
