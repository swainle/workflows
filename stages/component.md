...
只创建项目实际需要的文件。一个事实只由一个文件维护，其他文件通过稳定名称引用，不复制字段、关系或规则。契约由提供它的组件维护，消费方只能引用。Backend 需要引用前置文件的信息时，在所属二级或三级标题后、正文前使用独立的 `> Ref: <文件名>:<二级标题>:<三级标题或编号>`；多个引用各占一行。只有 `interface.md`“操作定义”的“关联需求”列可以直接列出当前操作实现的需求编号；除此之外不创建引用列、引用表或递归展开上游引用。
...
- `Design` 使用当前组件 Markdown 可解析的完整设计标识：文件名去掉扩展名，二、三级标题去掉
  `#` 和所有空白并以 `:` 连接；表格“编号”列或 `- **001**：` 标记追加三位编号，
  四级及更深标题不参与，缺少更深层级时停在当前章节。`Src` 使用组件应用目录下的
...
- 完整 DDD 模式的每个限界上下文必须包含领域模型、统一语言、业务规则和业务一致性；领域事件、状态图和时序图
  仅在实际存在时保留，各上下文独立维护自己的术语、规则、生命周期、协作和事件。
- `domain.md` 的每个时序图依次使用 `- **<流程>**` 说明引用对象、`> Ref: process:<角色>:<BP 编号>` 引用实际系统流程，再绘制 Mermaid；多个流程重复该结构，不增加四级标题。
...
- 领域模型表使用“类型、名称、所属聚合、职责、领域命令”五列；类型只使用中文的聚合根、实体、值对象、领域服务或领域策略。聚合根填写自身名称，实体只能属于一个聚合，聚合内部值对象填写聚合根名称，通用值对象填写“上下文共享”，领域服务和领域策略填写“上下文级”；跨聚合关系只持有对方聚合根的稳定 ID。“领域命令”通常由聚合根承担，只有确实协调多个领域对象的领域服务才直接承担命令，不是调用方或 Application Handler；不变量仍只在业务规则表维护。调用方由 `interface.md` 记录，Handler 和 Result 的代码映射由 `engineering.md` 和 `component.md` 维护。
...
- **Who**：处理 `<组件> [frontend|backend] opt <目标文件> [意见]` 的组件单文件优化 Agent。
- **When**：组件设计指令使用 `opt`，并指定当前组件设计目录内要创建或更新的一个 Markdown 文件时。
- **Where**：目标组件的前置规范、所选模板、目标文件及模板依赖顺序中位于目标之前的设计文件。
- **What**：提供“模板单文件优化”功能；具体规则、格式和约束如下。
- **Why**：确保优化只依据已经完成的上游设计，并且只创建或更新用户指定的当前文件。
...
| `docs/requirements/**` | 禁止 | 允许 | 禁止 | 禁止 |
| `docs/system/**` | 禁止 | 允许 | 禁止 | 禁止 |
| `<目标之前的设计依赖文件>` | 禁止 | 允许 | 禁止 | 禁止 |
| `<目标文件>` | 允许 | 允许 | 允许 | 禁止 |

- 只根据用户意见和前置事实优化目标文件，不从后续文件或当前实现反推设计。
- 除目标文件外，不创建、删除、移动、重命名或顺手修改模板、前置文件及关联文件。
- 目标修改要求后续文件同步时只报告影响，不在当前指令扩大读取或修改范围。
- 完成后执行目标文件适用的格式、引用和语法校验，并如实列出已读取文件、唯一修改文件和验证结果。
