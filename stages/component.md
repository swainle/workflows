...
Backend 按“C3 → DDD（含按需状态图与关键时序图）→ 接口和边界 → 机器可读模型 → C4 →
...
| `ddd.md` | 领域设计 | Backend 模式 | 按限界上下文维护边界、统一语言、领域模型图、按需状态图与关键时序图及领域事件 |
...
- `ddd.md` 按限界上下文分章，不复制 C4 中的类清单、字段、方法或关系。
- 一个组件默认对应一个限界上下文；只有组件内确实存在多个独立语言和模型边界时才分别列出，
  边界表明应拆分组件时切换 `[system]` 调整组件划分。
- 每个限界上下文必须包含边界、统一语言、领域模型图和领域事件列表；领域模型图必须表达
  聚合根、必要实体和值对象、关键不变量、事务边界、跨聚合引用和一致性方式。
- 聚合存在明确生命周期时才增加状态图；存在关键领域协作、事务边界或事件流转时才增加
  关键时序图，不为简单 CRUD 创建无意义的图。
...
- Backend 模式下，`ddd.md` 按限界上下文记录边界、统一语言、领域模型、按需状态图与关键时序图
