...
- `testing.md` 的每个测试分类先写目录路径，每个测试文件使用四级标题并记录测试对象；
  每个测试用例使用五级标题，格式为“`<用例编号> <简短中文描述>`”。
- 用例编号格式为“`<限界上下文>-<测试层级>-<对象或能力>-<三位序号>`”，在组件内唯一且
  保持稳定，删除后不复用；测试代码中的用例描述与标题一致。
- 每个用例使用可选的 `BP`、`Req`、`Design` 引用和必需的 `Given`、`When`、`Then`；
  `BP` 引用系统 `process.md` 中实际存在的跨组件业务流程，`Req` 引用实际存在的需求项，
  `Design` 引用实际设计文件及章节，三类来源至少存在一项。
...
- 引用的 BP、需求编号、权限编号、`operationId`、Design 章节和 Token 均存在。
