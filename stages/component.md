...
只创建项目实际需要的文件。一个事实只由一个文件维护，其他文件通过稳定名称引用，不复制字段、关系或规则。契约由提供它的组件维护，消费方只能引用。Backend 需要引用前置文件的信息时，在所属二级或三级标题后、正文前使用独立的 `> Ref: <文件名>:<二级标题>:<三级标题或编号>`；多个引用各占一行，不创建引用列、引用表或递归展开上游引用。
