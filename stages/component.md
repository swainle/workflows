...
Backend 按“C3 → DDD → 状态与组件时序 → 接口和边界 → 机器可读模型 → C4 →
配置、密钥、可观测性、测试与运行 → 部署交付 → component.md 汇总”的依赖顺序设计。
每个适用文件使用模板规定的标题名称和顺序；不适用的可选文件或章节直接省略，不创建空文件，
也不填写“无”“不适用”或其他占位正文。
...
| `ddd.md` | 领域设计 | Backend 模式 | 上下文边界、统一语言、聚合根、必要核心模型、不变量、事务边界和领域事件 |
| `state.md` | 后端状态模型 | 存在稳定状态模型 | 聚合、实体或任务的状态转换 |
| `sequence.md` | 组件内部调用时序 | 存在多方调用 | 系统流程引用、技术调用、事务、超时、重试和失败返回 |
...
- 当前组件的 `sequence.md` 引用系统流程的稳定名称，只表达入口、用例、领域对象、Port、
  Adapter 和外部系统之间的技术调用时序，不复制系统业务流程。
- 业务不变量、事务边界、一致性和补偿规则由 `ddd.md` 维护，状态转换和守卫条件由
  `state.md` 维护；`sequence.md` 只引用，不重复定义。
...
- `ddd.md` 只保存图无法完整表达的上下文边界、领域语义和决策，不复制 C4 中的类清单、字段、方法或关系。
- 一个组件默认对应一个上下文边界；只有组件内确实存在多个独立语言和模型边界时才分别列出，
  边界表明应拆分组件时切换 `[system]` 调整组件划分。
- 每个实际聚合必须明确聚合根、不变量和事务边界；核心成员只列理解聚合所必需的实体和值对象。
- “核心模型”只列具有独立身份、生命周期、重要等价规则或领域约束的实体和值对象；
  没有此类模型时删除该章节，不罗列数据载体、全部字段或实现类。
- 聚合根、实体、值对象、领域服务、仓储和领域事件的代码结构由 `c4.md` 表达；`ddd.md`
  使用相同稳定名称说明业务身份、等价规则、不变量和语义。
...
- Backend 模式下，`ddd.md` 只记录上下文边界、统一语言、聚合根、必要核心模型、不变量、
  事务边界、一致性与补偿、领域事件语义和建模决策，不复制 C4、DBML 或契约内容。
