...
- 领域模型表使用“类型、名称、职责、承担的领域命令”四列；类型只使用聚合根、实体、值对象、领域服务或领域策略。“承担的领域命令”表示拥有该业务行为和不变量的领域模型，不是调用方或 Application Handler；调用方由 `interface.md` 记录，Handler 和 Result 的代码映射由 `engineering.md` 和 `component.md` 维护。
