...
- `domain.md` 的每个时序图依次使用 `- **<流程>**` 说明引用对象、`> Ref: process:业务流程:<BP 编号>` 引用实际系统流程，再绘制 Mermaid；多个流程重复该结构，不增加四级标题。
...
- **When**：组件设计指令使用 `opt`，并指定当前组件设计目录内的一个现有 Markdown 文件时。
- **Where**：目标组件的前置规范、所选模板、目标文件及模板依赖顺序中位于目标之前的设计文件。
- **What**：提供“模板单文件优化”功能；具体规则、格式和约束如下。
- **Why**：确保优化只依据已经完成的上游设计，并且只修改用户指定的当前文件。
...
- 不创建、删除、移动、重命名或顺手修改模板、前置文件及关联文件。
