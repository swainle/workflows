...
阶段 AI 对置信度低于 95% 的事项使用对话确认：每次只问一个问题，等待回答后继续下一轮，直到关键事项全部确认。提问同时说明影响和原因，并给出一至三个参考答案；参考不代表人工确认。

确认完成后，AI 在阶段 Patch 中把本轮“问题、原因、参考、最终确认”写入需求目录的 `issue/questions.md`。该文件由所有阶段共享、自动加入后续 Prompt，人工通常不需要直接修改。问题应由其他阶段负责时，AI 会说明建议重跑的阶段。
