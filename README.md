...
能力分别维护领域、状态、内部调用时序、接口、数据、授权和界面规范，不创建重复的组件流程文件。
...
`[api] backend 设计组件` 默认采用 DDD，明确上下文边界、聚合根和必要核心模型，并将接口、认证、授权、领域、数据、校验、
错误、配置、密钥、可观测性、测试和运行部署要求分别写入单一职责文件。
Backend 模式完整读取 `templates/backend-design.template.md`，按“C3 → DDD → 状态与组件时序 →
