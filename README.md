...
组件名必须已登记在宿主项目的 `docs/system/system.md`。Frontend 与 Backend 模式只增强组件设计阶段，不是独立阶段。Frontend 按 `experience.md`、`state.md`、`ui/*.ui.yml`、`<组件>.design-token.json`、`configuration.md`、`testing.md` 和最终 `component.md` 的顺序设计；`component.md` 仅维护文件关系、概述和完整文件树。
...
跨组件同步 HTTP 契约唯一维护在 `docs/system/openapi.json`：`<system>` 先定义操作、Schema、示例和 `x-provider`，Backend `interface.md` 再映射并实现它，Frontend 通过 `operationId` 消费它，并在开发或测试环境用其 Schema 与示例驱动 Mock。
...
```text
提示词版本：<完整 SHA>（一致）
命中规则：AI-001、AI-004、AI-005、AI-006、AI-011、AI-TEST-001
```

版本门禁在读取项目业务文件前比较当前任务采用版本、宿主根 `AGENTS.md` 安装版本和本地工作流 HEAD；只有三个完整 SHA 一致时才执行指令。发生本地刷新时版本状态显示旧 SHA 和新 SHA。命中规则只列当前指令实际匹配的根提示词、阶段提示词和模式提示词；普通自然语言任务不强制回显。
