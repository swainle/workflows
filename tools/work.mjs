...
import { LEGACY_DESIGN_ARTIFACTS, REQUIREMENT_SPEC_ARTIFACTS } from "./core/specs.mjs";
...
const usage = `Usage:
  ...
  pnpm -s work:status
  pnpm -s work:patch [--require <text>] [--list] [--merge]
...
export function parseWorkArguments(args = process.argv.slice(2)) {
  ...
  if (command === "patch" || STAGE_BY_NAME[command]) {
    ...
    if (positionals.length || (values.list && values.require) || (values.merge && (values.list || values.require))) throw new Error(usage);
...
async function mergeActiveStage(current, stage) {
  ...
  for (const patchFile of pendingPatches) {
    checkPatch(current, patchFile === result.patchFile ? stage : "global", patchFile, {
      requireCompletion: stage === "patch" && patchFile === result.patchFile,
    });
  }
  if (stage === "patch" && !result.patchFile) {
    throw new Error(`Final Patch must create or update ${projectRelative(path.join(current.requirementDir, "completion.md"))}.`);
  }
  console.log(`Stage: ${stage}\n${result.noChanges ? "Result: no changes" : `Patches: ${patches.join(", ")}`}\nAnalysis: ${result.analysisFile}`);
  const answer = (await ask(`${pendingPatches.length ? "Apply and complete" : "Complete"} this stage? [y/N] `)).trim().toLowerCase();
  ...
  assertTrackingStageComplete(current, stage);
...
async function generatePatch(current, requirement = "") {
  ...
  for (const stage of ["design", "dev", "test"]) assertTrackingStageComplete(current, stage);
